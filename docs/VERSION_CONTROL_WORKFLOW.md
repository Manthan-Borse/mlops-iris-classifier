...
- **Repository:** https://github.com/Manthan-Borse/mlops-iris-classifier
- **Primary language:** Python
- **Maintainer(s):** Manthan Borse
...
- Practiced merge conflict resolution on `src/train.py` (both `conflict-demo-a`
  and `conflict-demo-b` changed the `n_estimators` value differently — 50 vs 200)
  rather than on `README.md` as in the base template. This surfaced a real
  post-merge bug: the manual resolution left one line in `main()` unindented,
  causing an `IndentationError` that only showed up when actually running the
  script — reinforcing why "test before pushing" (§5, step 7) matters.
- Resolved by keeping `n_estimators=200` and fixing the indentation in a
